...
def article_list(request):
    articles = Image.objects.all().order_by('date')
    return render(request, 'articles/article_list.html', { 'articles': articles })

def article_detail(request, slug):
    # return HttpResponse(slug)
    article = Image.objects.get(slug=slug)
...
def image_upload(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            inp=I.open(request.FILES['raw_image'])
            ...
            #print(type(inp))
            inp = imageio.imread( settings.BASE_DIR + image.raw_image.url)
            ...
            #return render(request, 'articles/image_display.html', { 'image': image, 'result':imm ,'media_url':settings.MEDIA_URL})
    else:
        form = forms.UploadImage()
    return render(request,'articles/image_upload.html', { 'form': form })

def image_display(request, name):
    # return HttpResponse(slug)
    image = Image.objects.get(name=name)
    return render(request, 'articles/image_display.html', { 'image': image })
